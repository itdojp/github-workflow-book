...
- [ ] 前提: 対象コミット（またはマージ済みPR）とリリース範囲が合意済み
- [ ] 必須チェック: main（またはリリース対象ブランチ）で必須チェックがすべて成功している
- [ ] バージョン/タグ: 命名規約（例: `vX.Y.Z`）とバージョン更新ルールが一致している（要確認: プロジェクトの規約）
- [ ] リリースノート: 自動生成/手動追記の方針が決まっている（機微情報を含めない）
- [ ] 成果物: ビルド手順が再現可能で、添付するファイルが最小になっている
- [ ] 権限: `permissions:` と Actions の既定権限を確認（`GITHUB_TOKEN` の書き込み可否）
- [ ] ロールバック: 失敗時の戻し方（再リリース手順、周知）を決めている
- [ ] リリース後: GitHub Release と成果物の確認、主要導線のsmoke test、監視確認を実施
