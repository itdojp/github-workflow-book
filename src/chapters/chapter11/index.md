...
### 11.1.1 エージェント運用の権限境界（最小権限）
エージェント（coding agent / third-party agents）に作業を任せるほど、**権限とSecretsの露出面**が増えます。運用で守るべき線引き（権限境界）を先に決めておくことが、最も効果の高い対策です。

#### 線引き（最低限）
- **デフォルトは読み取り**: 日常のCIは `contents: read` を基本にし、書き込み（PR作成/コメント投稿/マージ等）は必要なワークフローにだけ付与する
- **権限はジョブ単位で最小化**: `permissions:` を明示し、`write-all` 相当を避ける
- **トリガーを絞る**: `workflow_dispatch` や、メンテナーのみが付けられるラベル/承認を条件にする（外部PRやフォーク起点の自動実行は避ける）
- **保護ルールと組み合わせる**: branch protection / rulesets / CODEOWNERS / required checks により、最終承認を人間側に固定する
...
注意：`pull_request_target` はSecrets/トークンの扱いを誤ると外部からの悪用に繋がります。利用する場合は「何をチェックアウトし、何にSecretsを渡すか」を設計として明文化してください。

### 11.1.2 Secretsの露出面（ログ/PR/Artifacts）と対策
Secretsは「漏えい経路」を把握し、露出面を減らします。エージェント運用では特に次が重要です。

- **ワークフロー実行ログ**: `set -x` やデバッグ出力で環境変数が出ないようにする
- **PR本文/コメント**: エージェントが要約や差分説明を書く際に、値を貼り付けない（URLクエリ等も含む）
- **Artifacts/キャッシュ**: 生成物にSecretsが混入しない（設定ファイル、`.env`、ログファイル）

対策（最低限）:
- Secretsは **環境変数として参照するだけ**にし、標準出力へ出さない
- 例外ログにSecretsが混ざり得る箇所は、出力内容をレビュー対象に含める
- エージェントに与える入力（Issue/PR本文）に機密情報を書かない運用にする

### 11.1.3 監査・証跡（最低限）
「誰が、何を根拠に、何を変更したか」を残す設計が必要です。最低限、次を揃えると監査可能性が上がります。

- **Issueを実行仕様として残す**（受入基準/制約/変更禁止領域/テスト）
- **PRテンプレで検証結果を固定**（テスト、リスク、ロールバック、AI/エージェント利用の開示）
- **GitHub Actionsの実行ログを根拠として紐付ける**（該当run URLをPRに残す）
- 利用できる場合は **監査ログ（Audit log）** を使い、権限変更やSecrets操作の履歴を追える状態にする
...
過去のセキュリティアドバイザリは[こちら](advisories)で確認できます。
