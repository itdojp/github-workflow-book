...
## Description
現在のデータローダーには、データ拡張機能がありません。
学習データの多様性を増やすため、以下の拡張を追加したいです。
...
torchvision.transformsを使用して実装予定
...
#### バグ報告の例
```markdown
Title: Model fails with batch size 1

## Bug Description
バッチサイズを1に設定すると、モデルの推論時にエラーが発生します。
...
## Possible Fix
BatchNorm層でtrack_running_stats=Trueを設定する必要がある
